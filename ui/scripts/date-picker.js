...
function AddCalendarDays(Year, Month, Object) {
    ...
    var HeaderRow = `
        <table>
            <tr>
                <td class='date-picker-clickable' onclick='AddCalendarDays(window.Year, window.Month - 1);'>❮</td>
                <td class='date-picker-clickable' onclick='AddCalendarMonths(window.Year);'>${ window.Months[window.Month] + " " + window.Year.toString() }</td>
                <td class='date-picker-clickable' onclick='AddCalendarDays(window.Year, window.Month + 1);' >❯</td>
            </tr>
        </table>
    `;
    ...
    for (r = 0; r < 6; r++) {
        ...
        if (!Done) {
            ...
            for (c = 0; c < 7; c++) {
                ...
                MainContent += "<td class='date-picker-clickable' onclick='SetDate(this);'>";
...
function AddCalendarMonths(Year) {
    ...
    var HeaderRow = `
        <table>
            <tr>
                <td class='date-picker-clickable' onclick='AddCalendarMonths(window.Year - 1);'><</td>
                <td class='date-picker-clickable'>${ window.Year.toString() }</td>
                <td class='date-picker-clickable' onclick='AddCalendarMonths(window.Year + 1);' >></td>
            </tr>
        </table>
    `;
    ...
    for (r = 0; r < 3; r++) {
        ...
        for (c = 0; c < 4; c++) {
            var MonthNo = (r * 4) + c;
            MainContent += "<td class='date-picker-clickable' onclick='AddCalendarDays(window.Year, " + MonthNo + ", null);'>";
...
function SetDate(Obj) {
    if (Obj.innerHTML.toString() != '') {
        ...
        let day = parseInt(Obj.innerHTML);
